...
#define debug_print_cell(arr, length, cells_per_line)             \
	...
	cout << "\n====" << #arr << " end====" << endl;
#define debug_print_msg(msg) cout << msg << endl;
...
void record_possibility(int constrained_cells[MAX_CELLS][2],
	...
	int& lower_bound = num_flags_bounds_used[0];
	int& upper_bound = num_flags_bounds_used[1];
	int& lowest_num_flag = num_flags_bounds_used[2];
	int& highest_num_flag = num_flags_bounds_used[3];
	int used_flag = 0;
...
void handle_unconstrained_cells(int unconstrained_cells[MAX_CELLS][2],
	...
	if (highest_num_flag == lowest_num_flag &&
		highest_num_flag == upper_bound) {
		debug_print_msg("handle_unconstrained_cells: insert to reveal");
		FOR_EACH_UNCONSTRAINED_CELL {
			arr_2d_insert(cells_to_reveal, num_to_reveal++, row, col);
		}
	} else if (highest_num_flag == lowest_num_flag &&
			   lowest_num_flag == lower_bound) {
		debug_print_msg("handle_unconstrained_cells: insert to flag");
...
bool global_solver(int cells_to_reveal[MAX_CELLS][2],
	...
	int& constrained_length = cell_type_index[0];
	int& unconstrained_length = cell_type_index[1];

	classify_hidden_cells(constrained_cells, unconstrained_cells,
						  cell_type_index);

	debug_print_cell(constrained_cells, constrained_length, 20);
	debug_print_cell(unconstrained_cells, unconstrained_length, 20);
	debug_print_num("constrained_length", constrained_length);
	debug_print_num("unconstrained_length", unconstrained_length);
	...
	lower_bound = num_mine - unconstrained_length;
	...
	if (constrained_length)
		enumerate_possibilities(constrained_cells, 0, constrained_length,
								num_flags_bounds_used, num_safe_config_records,
								num_flagged_config_records);

	handle_constrained_cells(
		constrained_cells, constrained_length, num_safe_config_records,
		num_flagged_config_records, cells_to_reveal, cells_to_flag, num_cells);

	debug_print_num("enter handling unconstrained.\n\nlowest flag:",
					num_flags_bounds_used[2]);
	debug_print_num("highest flag:", num_flags_bounds_used[3]);
	handle_unconstrained_cells(unconstrained_cells, unconstrained_length,
