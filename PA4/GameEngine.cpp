			...
			if (players[p]->has_units_alive()) {
				...
				while (current_player->has_units_ready()) {
					...
					case '1':
						...
						if (is_game_over()) { return; } // Units could only die after attacking. Event-based Programming would be more efficient.
						break;
...
void GameEngine::select_unit() {
	...
	for (int i = 0; i < current_player->get_num_units(); i += 1) {
		...
		if (unit->is_alive() && unit->is_ready()) { selection_units_id += unit->get_id(); }
	}
	...
	do {
		...
		cin >> selection_id;
	} while (selection_units_id.find(selection_id) == string::npos); // string::npos signifies not found.
...
void GameEngine::process_unit_movement_phase() {
	...
	do {
		cout << "Enter movement delta (row col), input (0 0) to Hold Position: ";
		cin >> delta_row >> delta_col;
		if ((delta_row == 0) && (delta_col == 0)) { return; }
	} while (!game_map.is_valid_path(starting_row, starting_col, delta_row, delta_col, current_unit->get_movement_range()));
...
void GameEngine::process_unit_attack_phase() {
	...
	for (int p = 0; p < num_players; p += 1) {
		if (players[p] == current_player) { continue; }
		for (int u = 0; u < players[p]->get_num_units(); u += 1) {
			Unit* enemy_unit = players[p]->get_units()[u];
			if (enemy_unit->is_alive()) {
				int distance_row = ((current_unit->get_position_row() > enemy_unit->get_position_row()) ? (current_unit->get_position_row() - enemy_unit->get_position_row()) : (enemy_unit->get_position_row() - current_unit->get_position_row()));
				int distance_col = ((current_unit->get_position_col() > enemy_unit->get_position_col()) ? (current_unit->get_position_col() - enemy_unit->get_position_col()) : (enemy_unit->get_position_col() - current_unit->get_position_col()));
				if ((distance_row + distance_col) <= current_unit->get_attack_range()) { enemy_units_in_range_id += enemy_unit->get_id(); }
			}
		}
	}

	if (enemy_units_in_range_id == "") {
		cout << "No Enemy Units within Attack Range. Ending Turn for Unit " << current_unit->get_id() << "." << endl;
		return;
	}

	cout << "Select Enemy Unit to Attack [" << enemy_units_in_range_id << "], or input / to Skip Attack: ";
	char selection_id;
	cin >> selection_id;
	if (enemy_units_in_range_id.find(selection_id) != string::npos) {
		...
		for (int p = 0; p < num_players; p += 1) {
			...
			if (enemy_unit != nullptr) { break; }
		}

		current_unit->attack_unit(enemy_unit);
		if (!current_unit->is_alive()) { game_map.update_terrain_map(current_unit->get_position_row(), current_unit->get_position_col(), GameMap::TerrainState::EMPTY); }
		if (!enemy_unit->is_alive()) { game_map.update_terrain_map(enemy_unit->get_position_row(), enemy_unit->get_position_col(), GameMap::TerrainState::EMPTY); }
	}
}
